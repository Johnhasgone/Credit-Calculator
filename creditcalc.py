...
if args.type != "annuity" and args.type != "diff" or \
        ...
        args.type == "diff" and args.payment or len(arg_list) != 5\
    ...
    if not args.periods:
        periods = math.ceil(
            math.log(args.payment / (args.payment - args.interest / 1200 * args.principal), 1 + args.interest / 1200))
        years = periods // 12
        months = periods % 12
        ...
        if years == 0:
            ...
            print(f"You need {years} year{plural_y} and {months} month{plural_y} to repay this credit!")
        overpayment = periods * args.payment - args.principal

    elif not args.payment:
        if args.type == "annuity":
            annuity_payment = math.ceil(
                args.principal * (args.interest / 1200 * (1 + args.interest / 1200) ** args.periods) / \
            ...
            for i in range(1, args.periods + 1):
                monthly_payment = math.ceil(args.principal / args.periods + args.interest / 1200 * \
        ...
        principal = int(args.payment / ((args.interest / 1200 * ((1 + args.interest / 1200) ** args.periods)) / (
                    ((1 + args.interest / 1200) ** args.periods) - 1)))
